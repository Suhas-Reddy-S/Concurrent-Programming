...
static mutex pthread_lk; // Mutex for bucket access
...
static void insert(uint32_t* in_arr, map<uint32_t, uint32_t>* buckets, const uint32_t* mins, size_t k, queue<bucket_sort_params_t>& tasks, reverse_sense_barrier &bar_sense) {
	while (true) {
...
void bucket_sort_bar_sense(uint32_t* in_arr, size_t len, size_t k, size_t num_threads) {
	...
	for (size_t i = 0; i < num_threads; ++i) {
		threads[i] = thread(insert, in_arr, buckets, mins, k, ref(tasks), ref(bar_sense));
		#if DEBUG_MODE
