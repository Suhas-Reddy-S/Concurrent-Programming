...
static void insert(uint32_t* in_arr, map<uint32_t, uint32_t>* buckets, const uint32_t* mins, size_t k, queue<bucket_sort_params_t>& tasks, barrier<> &bar_pthread) {
	while (true) {
...
void bucket_sort_bar_pthread(uint32_t* in_arr, size_t len, size_t k, size_t num_threads) {
	...
	for (size_t i = 0; i < num_threads; ++i) {
		threads[i] = thread(insert, in_arr, buckets, mins, k, ref(tasks), ref(bar_pthread));
		#if DEBUG_MODE
